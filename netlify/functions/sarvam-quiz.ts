...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('[Sarvam Quiz] No API key found');
      return {
        statusCode: 500,
        headers,
...
{"questions":[{"question":"What is photosynthesis?","options":["Food making","Breathing","Growing","Moving"],"answer":0}]}` 
      ...
      console.log(`[Sarvam Quiz] Attempt ${attempt}/${maxAttempts} - Sending request to API`);
      ...
      console.log("[Sarvam Quiz] Raw API response:", text);
      ...
      console.log("[Sarvam Quiz] AI content:", content);
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.log("[Sarvam Quiz] Successfully generated quiz on attempt", attempt);
        return {
          statusCode: 200,
          headers,
        ...
        lastError = err;
        console.error(`[Sarvam Quiz] Attempt ${attempt} failed:`, err.message);
        if (attempt < 3) {
    ...
    console.error("[Sarvam Quiz] All quiz generation attempts failed");
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({
    ...
    console.error('[Sarvam Quiz] Unexpected error:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
