...
export function ToolsShowcase() {
  // Pause TTS
  const { cancel: sarvamPause } = useSarvamTTS();
  ...
  const [aiSummary, setAISummary] = useState<string>("");
    const generateAISummary = async (text?: string) => {
      // Replace with your AI summary API call
      // Example: POST to /api/dyslexia-summary with inputText
      const summaryText = text ?? inputText;
      setAISummary("This is a dyslexia-friendly summary generated by AI. Key ideas are short, clear, and easy to read.\n\nInput: " + summaryText.slice(0, 200));
  };
  // Sarvam AI TTS hook
  const { speak: sarvamSpeak, isSpeaking: isSarvamSpeaking, audioUrl: sarvamAudioUrl } = useSarvamTTS();
  ...
  const { speak, cancel, isSpeaking, audioUrl } = useSpeechSynthesis();
  const {
